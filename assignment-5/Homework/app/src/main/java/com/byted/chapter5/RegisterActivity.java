    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // todo 做网络请求
                apiService.postRegister(name, password, repassword).enqueue(new Callback<UserResponse>() {
                    @Override
                    public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                        if (response.body() != null) {
                            if (response.body().errorCode == 0) {
                                Toast.makeText(RegisterActivity.this, "注册成功，昵称为 " + response.body().user.nickname, Toast.LENGTH_SHORT).show();
                            }
                            else {
                                Toast.makeText(RegisterActivity.this, "失败，" + response.body().errorMsg, Toast.LENGTH_SHORT).show();
                                // Log.d("RGIS", response.body().errorMsg);
                            }
                        }
                    }
